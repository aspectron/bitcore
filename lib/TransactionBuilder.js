...
TransactionBuilder._scriptForOut = function(out, litecoin) {
  var ret;
  if (out.address)
    ret = new Address(out.address, litecoin?'ltc':false).getScriptPubKey();
...
TransactionBuilder.infoForP2sh = function(opts, networkName, litecoin) {
  var script = this._scriptForOut(opts, litecoin?'ltc' : false);
  var hash = util.sha256ripe160(script.getBuffer());

  if(litecoin){
    networks.testnet = networks['ltc'].testnet;
    networks.livenet = networks['ltc'].livenet;
  }

  var version = networkName === 'testnet' ?
    networks.testnet.P2SHVersion : networks.livenet.P2SHVersion;

  var addr = new Address(version, hash, litecoin?'ltc':false);
...
TransactionBuilder.prototype._setRemainder = function(txobj, remainderIndex) {
  ...
  if (remainderSat.cmp(0) > 0) {
    ...
    var script = TransactionBuilder._scriptForOut(remainderOut, this.litecoin);
    var txout = {
      v: value,
      s: script.getBuffer(),
    };
...
TransactionBuilder.prototype._setFeeAndRemainder = function(txobj) {
  ...
  if(this.litecoin){
    FEE_PER_1000B_SAT = parseInt(0.001 * util.COIN);
  }  
...
TransactionBuilder.prototype.setOutputs = function(outs) {
  ...
  for (var i = 0; i < l; i++) {
    ...
    var script = TransactionBuilder._scriptForOut(outs[i], this.litecoin);
    var txout = {
      v: value,
      s: script.getBuffer(),
    };
...
TransactionBuilder._mapKeys = function(keys, litecoin) {
  /* prepare keys */
  var walletKeyMap = {};
  var l = keys.length;
  var wk;
...
TransactionBuilder.prototype._multiFindKey = function(walletKeyMap, pubKeyHash) {
  ...
  [networks.livenet, networks.testnet].forEach(function(n) {
    [n.addressVersion, n.P2SHVersion].forEach(function(v) {
      var a = new Address(v, pubKeyHash, self.litecoin?'ltc':false);
...
TransactionBuilder.prototype.sign = function(keys) {
  ...
  var tx = this.tx,
    ins = tx.ins,
    l = ins.length,
    walletKeyMap = TransactionBuilder._mapKeys(keys, this.litecoin);
