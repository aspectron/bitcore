...
function rpc(request, callback) {
  ...
  var coin = 'bitcoin';
  if(self.port == 9332 || self.port == 19332 || self.port == 9333 || self.port == 19333){
    coin = 'litecoin';
  }
  ...
  var req = this.protocol.request(options, function(res) {
    ...
    res.on('end', function() {
      if (res.statusCode == 401) {
        callback(new Error(coin + ' JSON-RPC connection rejected: 401 unauthorized'));
        return;
      }
      if (res.statusCode == 403) {
        callback(new Error(coin + ' JSON-RPC connection rejected: 403 forbidden'));
        return;
      }
  ...
  req.on('error', function(e) {
    var err = new Error('Could not connect to '+coin+' via RPC at host: ' + self.host + ' port: ' + self.port + ' Error: ' + e.message);
    log.err(err);
    callback(err);
  });
