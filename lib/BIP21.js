...
BIP21.prototype.parse = function(uri) {
  ...
  var coin = (info.protocol == 'bitcoin:' || info.protocol == 'litecoin:');
...
BIP21.prototype.setAddress = function(addr) {
  if (addr) {
    this.address = Address.validate(addr, this.coin) ? new Address(addr, this.coin) : addr;
  }
...
BIP21.prototype.getURI = function() {
  return URL.format({
    protocol: this.coin ? 'litecoin:' : 'bitcoin:',
    host: this.address,
    query: this.data
  });
}
