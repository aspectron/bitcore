...
function Address(version, hash, litecoin) {

    if (litecoin || arguments[1] == 'ltc') {
        this.litecoin = litecoin ? litecoin : hash;
        hash = litecoin ? hash : null;
    }
...
Address.fromPubKey = function(pubKey, network, litecoin) {        
    ...
    if (litecoin || arguments[1] == 'ltc') {
        ...
        network  = arguments[1] == 'ltc' ? 'livenet' : network;
    }

    if (litecoin)
        networks[network] = networks[litecoin][network];
    ...
    return new Address(version, hash, litecoin);
};

// create an address from a Key object
Address.fromKey = function(key, network, litecoin) {
    if (!network)
        network = 'livenet';
    ...
    return Address.fromPubKey(key.public, network, litecoin);
};

// create a p2sh m-of-n multisig address
Address.fromPubKeys = function(mReq, pubKeys, network, opts, litecoin) {
    if (!network)
        network = 'livenet';
    ...
    return Address.fromScript(script, network, litecoin);
};

//create a p2sh address from redeemScript
Address.fromScript = function(script, network, litecoin) {
    if (!network)
        network = 'livenet';
...
Address.fromScriptPubKey = function(scriptPubKey, network, litecoin) {
    ...
    if (payload) {
        ...
        for (var i in payload)
            ret.push(new Address(version, payload[i], litecoin));
    }
    return ret;
};
...
Address.prototype.network = function() {
    ...
    var livenet = this.litecoin ? networks[this.litecoin].livenet : networks.livenet;
    var testnet = this.litecoin ? networks[this.litecoin].testnet : networks.testnet;
...
Address.fromPubkeyHashScriptSig = function(scriptSig, network, litecoin) {
    return Address.fromPubKey(scriptSig.chunks[1], network, litecoin);
};

//extract an address from scriptSig
Address.fromScriptSig = function(scriptSig, network, litecoin) {
    ...
    if (scriptSig.chunks.length === 2)
        return Address.fromPubkeyHashScriptSig(scriptSig, network, litecoin);
    // TODO: support other scriptSig types 
    return null;
};

Address.getScriptPubKeyFor = function(s, litecoin) {
    return new Address(s, litecoin).getScriptPubKey();
};

Address.validate = function(s, litecoin) {
    return new Address(s, litecoin).isValid();
};
