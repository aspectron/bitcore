...
NetworkMonitor.create = function(config) {
  ...
  var peerman = new PeerManager({
    ...
    litecoin : this.config.litecoin ? true : false
  });
...
NetworkMonitor.prototype.init = function() {
  ...
  var handleTx = function(info) {
    ...
    var from = tx.getSendingAddresses(self.networkName, self.config.litecoin);
    for (var i = 0; i < from.length; i++) {
      var addr = from[i];
      self.emit('out:'+addr, tx);
    }
    var to = tx.getReceivingAddresses(self.networkName, self.config.litecoin); 
