...
NetworkMonitor.create = function(config) {
  ...
  var peerman = new PeerManager({
    ...
    litecoin : this.config.litecoin ? 'ltc' : false
  });

  peerman.addPeer(new Peer(config.host, config.port));
  return new NetworkMonitor(peerman, this.config.litecoin);
};
...
NetworkMonitor.prototype.init = function() {
  ...
  var handleTx = function(info) {
    ...
    var from = tx.getSendingAddresses(self.networkName, self.litecoin);
    for (var i = 0; i < from.length; i++) {
      var addr = from[i];
      self.emit('out:'+addr, tx);
    }
    var to = tx.getReceivingAddresses(self.networkName, self.litecoin); 
