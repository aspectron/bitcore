...
Transaction.prototype.fromObj = function fromObj(obj) {
  ...
  keys.forEach(function(addrStr) {
    var addr = new Address(addrStr, self.litecoin);
...
Transaction.prototype.getReceivingAddresses = function(networkName, litecoin) {
  if (!networkName) networkName = 'livenet';
  ret = [];
  for (var i = 0; i<this.outs.length; i++) {
    var o = this.outs[i];
    var addrs = Address.fromScriptPubKey(o.getScript(), networkName, litecoin);
...
Transaction.prototype.getSendingAddresses = function(networkName, litecoin) {
  if (!networkName) networkName = 'livenet';
  var ret = [];
  for (var i = 0; i<this.ins.length; i++) {
    ...
    var addr = Address.fromScriptSig(scriptSig, networkName, litecoin);
