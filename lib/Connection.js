...
function Connection(socket, peer, opts) {
  ...
  if(this.config.litecoin){
    PROTOCOL_VERSION = 70002;
    networks[this.config.network] = networks['ltc'][this.config.network];
  }
...
Connection.prototype.sendVersion = function() {
  var subversion =  this.config.litecoin ? '/LitecoinX:0.1/' : '/BitcoinX:0.1/';
...
Connection.prototype.parseMessage = function(command, payload) {
  ...
  switch (command) {
    ...
    case 'tx':
      ...
      tx.litecoin = this.config.litecoin ? 'ltc' : false;
      tx.parse(parser);
      return {
        command: command,
        version: tx.version,
